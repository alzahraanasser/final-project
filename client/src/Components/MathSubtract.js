...
function MathSubtract() {
  ...
  const [updatedContent, setUpdatedContent] = useState({
    title: 'الطرح',
    description: 'عند طرح رقم من آخر، نحذف الكمية المطلوبة لنحصل على الناتج.',
    example: '٥ - ٢ = ٣'
  });
  ...
  return (
    <Container className="mt-5 text-center">
      ...
      <h2 className="text-danger mb-4">➖ الطرح</h2>
      <Card className="shadow p-3">
        <CardBody>
          <img src={Subtract} alt="طرح" className="img-fluid mb-4" style={{ maxWidth: "300px" }} />
          ...
          <div className="mt-4 d-flex justify-content-center gap-3">
            <Button color="danger" onClick={() => navigate("/mathsubtractexercise")}>ابدأ التمرين</Button>
            {user?.role === "teacher" && (
              <>
      ...
      <Modal isOpen={modal} toggle={toggleModal}>
        <ModalHeader toggle={toggleModal}>تحديث درس الطرح</ModalHeader>
        <ModalBody>
          <Form>
            <FormGroup>
